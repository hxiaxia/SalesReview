...
### 🌟 完整全自动化流程 (包含从纷享销客抓取数据)
如果您希望从**登录抓取 -> 下载表单 -> 提取记录 -> 生成所有维度的 PDF 报告**一次性全自动完成，请直接运行主程序（确保您已经配置了 `config.json` 里的账号密码）：
```bash
python main.py
```
*(注：首次运行时浏览器可能会弹出验证码，需手动拼图，通过后系统会自动记录凭证，后续皆为无人值守。)*

---

### 单独生成各维度报告 (离线处理)
如果数据已经抓取完毕并存放在 `downloads/` 文件夹中，您也可以单独执行分析脚本。系统会自动寻找最新下载的 Excel（或者通过命令行参数直接指定文件路径）。
